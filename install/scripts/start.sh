...
function start_shibd {
    /usr/sbin/shibd -u $SHIBDUSER -g root -p /var/run/shibboleth/shib.pid > /var/log/shibboleth/start.log 2>&1
}


function start_httpd {
    # do not start with root to avoid permission conflicts on log files
    su - $HTTPDUSER  -c 'httpd -DFOREGROUND -d /etc/httpd/ -f conf/httpd.conf'  >> /var/log/httpd/start.log 2>&1
}
